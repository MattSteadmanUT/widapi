...
/// <summary>WID 3.0 LaborForce table (BLS LAUS data).</summary>
[ApiController]
[Authorize]
[Route("labor-force")]
[Route("laborForce")]
public sealed class LaborForceController(
    WIDDbContext db,
...
{
    /// <summary>
    /// Returns a paged list of LaborForce (LAUS) records matching the supplied filters.
    /// Supports CSV multi-value filters, min/max year range, user-defined sort,
    /// and download in CSV / TSV / PSV / XLSX / JSON formats.
    /// </summary>
    [HttpGet]
    ...
    {
        IQueryable<LaborForce> query = db.LaborForce.AsNoTracking();
        ...
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.AreaType), areaType);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.AreaTypeVersion), areaTypeVersion);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.Area), area, WidCodes.NormalizeArea);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.PeriodYear), periodYear);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.PeriodType), periodType);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.Period), period);
        query = QueryFiltering.ApplyStringFilter(query, nameof(LaborForce.Adjusted), adjusted);
        ...
        var ordered = QuerySorting.Apply(
            query,
            sort,
            nameof(LaborForce.StFips), nameof(LaborForce.AreaType), nameof(LaborForce.AreaTypeVersion), nameof(LaborForce.Area),
            nameof(LaborForce.PeriodYear), nameof(LaborForce.PeriodType), nameof(LaborForce.Period), nameof(LaborForce.Adjusted));
        ...
        if (resolvedFormat != DownloadFormat.Json)
        {
            ...
            {
                DownloadFormat.Tsv      => downloadService.TsvResult(rows, "labor-force"),
                DownloadFormat.Psv      => downloadService.PsvResult(rows, "labor-force"),
                DownloadFormat.JsonFile => downloadService.JsonFileResult(rows, "labor-force"),
                DownloadFormat.Xlsx     => downloadService.XlsxResult(rows, "labor-force"),
                _                       => downloadService.CsvResult(rows, "labor-force"),
            };
        }
    ...
    /// <summary>
    /// Returns coverage metadata for the LaborForce table under the supplied filters.
